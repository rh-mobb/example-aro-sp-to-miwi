# ARO SP to MI/WI Migration Demo

Hands-on walkthrough for migrating two demo applications from an Azure Red Hat OpenShift (ARO) **Service Principal cluster** to a **Managed Identity cluster** with Workload Identity.

Part of the [ARO Service Principal to Managed Identity](https://cloud.redhat.com/experts/aro/aro-sp-mi/) guide on Red Hat Cloud Experts. Read that article first — it explains what changes in authentication and why.

## What This Demo Covers

Two Python Flask applications that access Azure services:

**keyvault-reader** — reads secrets from Azure Key Vault. Demonstrates the most common migration case: changing from `ClientSecretCredential` to `DefaultAzureCredential` (a one-line code change) and replacing the K8s Secret with a ServiceAccount annotation.

**blob-writer** — writes entries to Azure Blob Storage. Demonstrates that apps already using `DefaultAzureCredential` need no code change at all; only K8s manifest changes are needed.

| Demo App | Azure Service | SP Auth Method | Code Change? | Config Change? |
|----------|--------------|----------------|-------------|---------------|
| **keyvault-reader** | Azure Key Vault | `ClientSecretCredential` with K8s Secret | **Yes**: one-line change to `DefaultAzureCredential` | **Yes**: replace Secret with ServiceAccount |
| **blob-writer** | Azure Blob Storage | `DefaultAzureCredential` with K8s Secret | **No**: SDK auto-detects workload identity | **Yes**: replace Secret with ServiceAccount |
...
```mermaid
graph LR
    subgraph before["ARO SP Cluster — Before"]
        KVR_SP["keyvault-reader\nClientSecretCredential · K8s Secret ⚠"]
        BW_SP["blob-writer\nDefaultAzureCredential · K8s Secret ⚠"]
    end

    subgraph after["ARO MI Cluster — After"]
        KVR_MI["keyvault-reader\nDefaultAzureCredential · ServiceAccount ✓"]
        BW_MI["blob-writer\nDefaultAzureCredential · ServiceAccount ✓"]
    end
...
This section describes how the two demo applications are configured on an existing SP cluster. If you already have applications running on your SP cluster, these patterns will look familiar: a K8s Secret holding Azure credentials, injected into pods via `envFrom`.
...
**Application code (`keyvault-reader/app_sp.py`):**
...
**Application code (`blob-writer/app.py`):**
...
After migration, both apps run on the MI cluster using workload identity — no Azure secrets stored in the cluster.
...
> [!NOTE]
> The OIDC issuer URL is only available on MI clusters. If this returns empty, the cluster is SP-based and does not support workload identity.
...
> [!NOTE]
> The `--subject` format is `system:serviceaccount:<namespace>:<serviceaccount-name>`. This must exactly match the ServiceAccount in your K8s manifests.
...
```bash
git clone https://github.com/rh-mobb/example-aro-sp-to-miwi.git
cd example-aro-sp-to-miwi
```
...
oc start-build keyvault-reader --from-dir=keyvault-reader -n keyvault-reader --follow
...
oc start-build blob-writer --from-dir=blob-writer -n blob-writer --follow
...
- [ARO Service Principal to Managed Identity (Part 1)](https://cloud.redhat.com/experts/aro/aro-sp-mi/)
